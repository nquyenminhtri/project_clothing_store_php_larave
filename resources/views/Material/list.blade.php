...
@section('content')
    ...
    <button type="button" class="waves-effect waves-light btn-primary btn-outline-primary btn btn-primary btn-click"
        ...
        <i class="icofont icofont-user-alt-3"></i>Create new material
    </button>
    <!-- Modal -->
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Create new material</h5>
                        ...
                        @foreach ($materialList as $material)
                            <tr>
                                ...
                                <td style="display:flex;align-items: center;">
                                    <a href="#" onclick="setModalAction('edit', {{ $material->id }})"><button
    ...
    <script>
        ...
        function setModalAction(action, materialId = null) {

            if (action === 'create') {
                // Nếu là tạo mới, đặt lại biểu mẫu và thay đổi tiêu đề modal
                $('#materialId').val('');
                $('#actionType').val('create');
                $('#CreateOrUpdateForm').attr('action', '{{ route('material.handle-create') }}');
                $('#CreateOrUpdateForm')[0].reset();
                $('#CreateOrUpdateForm input[name="password"]').closest('.form-group').show();
                $('#exampleModalLabel').text('Create a new material');
                $('#btnActionText').text('Create');
            } else if (action === 'edit') {
                $('#actionType').val('edit');
                $('#materialId').val(materialId);
                $('#CreateOrUpdateForm').attr('action', '{{ url('material/update') }}/' + materialId);
                $('#exampleModalLabel').text('Edit material');
                $('#btnActionText').text('Save');
                // Lấy dữ liệu bằng Ajax

                $.ajax({
                    type: 'GET',
                    url: '{{ url('material/update') }}/' + materialId,
        ...
        function submitForm() {
            ...
            var url = (actionType === 'create') ? '{{ route('material.handle-create') }}' :
                '{{ url('material/update') }}/' + materialId;
        ...
        function fillDataTable(data) {
            ...
            var deleteRoute = '{{ route('material.delete', ['id' => ':id']) }}';
            var deleteMethod = '{{ method_field('DELETE') }}';
            $.each(data, function(index, material) {
                var deleteUrl = deleteRoute.replace(':id', material.id);
                var row = '<tr>' +
                    '<td>' + material.id + '</td>' +
                    '<td>' + material.name + '</td>' +
                    '<td style="display:flex;align-items: center;">' +
                    '<a href="#" onclick="setModalAction(\'edit\', ' + material.id + ')">' +
                    '<button type="button" class="btn btn-warning">Edit</button>' +
                    '</a> | ' +
                    '<form id="deleteForm_' + material.id +
                    '" onsubmit="return confirm(\'Are you sure you want to delete this material?\');" >' +
                    '@csrf' +
                    '@method('DELETE')' +
                    '<button type="button" class="btn btn-danger" onclick="handleDelete(' + material.id +
                    ')">Delete</button>' +
                    '</form>' +
                    '</td>' +
                    '</tr>';
