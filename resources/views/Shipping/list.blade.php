...
@section('content')
    <div style="margin-left:1%;withd:100%;height:50px;display:flex; margin-top:-20px" class="row">
        <h4 style="width:50%;">List shipping</h4>
    ...
    <button type="button" class="waves-effect waves-light btn-primary btn-outline-primary btn btn-primary btn-click"
        ...
        <i class="icofont icofont-user-alt-3"></i>Create new shipping
    </button>
    <!-- Modal -->
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Create new shipping</h5>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="cost" class="form-control"
                                    placeholder="Enter cost shipping method !" required>
                                <div class="error-message"></div>
                            </div>
                        </div>
                    </form>
                        ...
                        @foreach ($shippingList as $shipping)
                            <tr>
                                ...
                                <td style="display:flex;align-items: center;">
                                    <a href="#" onclick="setModalAction('edit', {{ $shipping->id }})"><button
    ...
    <script>
        ...
        function setModalAction(action, shippingId = null) {
            if (action === 'create') {
                // Nếu là tạo mới, đặt lại biểu mẫu và thay đổi tiêu đề modal
                $('#shippingId').val('');
                $('#actionType').val('create');
                $('#CreateOrUpdateForm').attr('action', '{{ route('shipping.handle-create') }}');
                $('#CreateOrUpdateForm')[0].reset();
                $('#exampleModalLabel').text('Create a new shipping method');
                $('#btnActionText').text('Create');
            } else if (action === 'edit') {
                $('#actionType').val('edit');
                $('#shippingId').val(shippingId);
                $('#CreateOrUpdateForm').attr('action', '{{ url('shipping/update') }}/' + shippingId);
                $('#exampleModalLabel').text('Edit shipping');
                $('#btnActionText').text('Save');
                // Lấy dữ liệu bằng Ajax

                $.ajax({
                    type: 'GET',
                    url: '{{ url('shipping/update') }}/' + shippingId,
                    success: function(response) {
                        console.log(response.data.name);
                        // Điền vào biểu mẫu với dữ liệu
                        $('#CreateOrUpdateForm input[name="name"]').val(response.data.name);
                        $('#CreateOrUpdateForm input[name="cost"]').val(response.data.cost);
                    },
        ...
        function submitForm() {
            ...
            var url = (actionType === 'create') ? '{{ route('shipping.handle-create') }}' :
                '{{ url('shipping/update') }}/' + shippingId;
        ...
        function handleDelete(shippingId) {
            $.ajax({
                type: 'DELETE',
                url: '/shipping/delete/' + shippingId,
                data: {
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            text: 'Shipping method deleted success!',
                            timer: 1300
                        });
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error!',
                            text: 'Shipping method deleted failed!',
                            timer: 1300
                        });
                    }
                },
        ...
        function fillDataTable(data) {
            ...
            var deleteRoute = '{{ route('shipping.delete', ['id' => ':id']) }}';
            var deleteMethod = '{{ method_field('DELETE') }}';
            $.each(data, function(index, shipping) {
                var deleteUrl = deleteRoute.replace(':id', shipping.id);
                var row = '<tr>' +
                    '<td>' + shipping.id + '</td>' +
                    '<td>' + shipping.name + '</td>' +
                    '<td>' + shipping.cost + '</td>' +
                    '<td style="display:flex;align-items: center;">' +
                    '<a href="#" onclick="setModalAction(\'edit\', ' + shipping.id + ')">' +
                    '<button type="button" class="btn btn-warning">Edit</button>' +
                    '</a> | ' +
                    '<form id="deleteForm_' + shipping.id +
                    '" onsubmit="return confirm(\'Are you sure you want to delete this shipping?\');" >' +
                    '@csrf' +
                    '@method('DELETE')' +
                    '<button type="button" class="btn btn-danger" onclick="handleDelete(' + shipping.id +
                    ')">Delete</button>' +
                    '</form>' +
                    '</td>' +
                    '</tr>';
