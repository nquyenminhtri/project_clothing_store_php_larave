...
@section('content')
    <div style="margin-left:1%;withd:100%;height:50px;display:flex; margin-top:-20px" class="row">
        <h4 style="width:50%;">List promotion</h4>
    ...
    <button type="button" class="waves-effect waves-light btn-primary btn-outline-primary btn btn-primary btn-click"
        ...
        <i class="icofont icofont-user-alt-3"></i>Create new promotion
    </button>
    <!-- Modal -->
                        ...
                        @foreach ($promotionList as $promotion)
                            <tr>
                                ...
                                <td style="display:flex;align-items: center;">
                                    <a href="#" onclick="setModalAction('edit', {{ $promotion->id }})"><button
                                            type="button" class="btn btn-warning">Edit</button></a>
                                    |
                                    <form method="POST" action="{{ route('promotion.delete', ['id' => $promotion->id]) }}">
                                        @csrf
    ...
    <script>
        ...
        function setModalAction(action, promotionId = null) {

            if (action === 'create') {
                // Nếu là tạo mới, đặt lại biểu mẫu và thay đổi tiêu đề modal
                $('#promotionId').val('');
                $('#actionType').val('create');
                $('#CreateOrUpdateForm').attr('action', '{{ route('promotion.handle-create') }}');
                ...
                $('#CreateOrUpdateForm').attr('action', '{{ url('promotion/update') }}/' + promotionId);
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ url('promotion/update') }}/' + promotionId,
        ...
        function submitForm() {
            ...
            var url = (actionType === 'create') ? '{{ route('promotion.handle-create') }}' :
                '{{ url('promotion/update') }}/' + promotionId;
        ...
        function handleDelete(promotionId) {
            $.ajax({
                type: 'DELETE',
        ...
        function fillDataTable(data) {
            ...
            var deleteRoute = '{{ route('promotion.delete', ['id' => ':id']) }}';
            var deleteMethod = '{{ method_field('DELETE') }}';
            $.each(data, function(index, promotion) {
                var deleteUrl = deleteRoute.replace(':id', promotion.id);
                var row = '<tr>' +
                    '<td>' + promotion.id + '</td>' +
                    '<td>' + promotion.name + '</td>' +
                    '<td style="display:flex;align-items: center;">' +
                    '<a href="#" onclick="setModalAction(\'edit\', ' + promotion.id + ')">' +
                    '<button type="button" class="btn btn-warning">Edit</button>' +
                    '</a> | ' +
                    '<form id="deleteForm_' + promotion.id +
                    '" onsubmit="return confirm(\'Are you sure you want to delete this promotion?\');" >' +
                    '@csrf' +
                    '@method('DELETE')' +
                    '<button type="button" class="btn btn-danger" onclick="handleDelete(' + promotion.id +
                    ')">Delete</button>' +
                    '</form>' +
                    '</td>' +
                    '</tr>';
